...
function SectionHead({ num, eyebrow, title, en, accent = "silver", accentSize = 60, accentStyle = {} }) {
  return (
    <div className="section-head">
      ...
      <h2 className="display" style={{ height: "115px" }}>{title}</h2>
      <div style={{ display: "flex", alignItems: "center", gap: 18, marginTop: -4 }}>
        <TitleAccent variant={accent} size={accentSize} style={accentStyle} />
        <div className="eyebrow-latin" style={{ alignSelf: "end", fontSize: "36px" }}>{en}</div>
      </div>
    </div>);
...
function PageHero({ num, eyebrow, title, lead, en }) {
  return (
    <section style={{
      ...
      <div className="pattern-band side-left" style={{ width: "40%", opacity: 0.12 }}></div>
      <div className="navy-light-sweep"></div>
      <DecorPattern />
