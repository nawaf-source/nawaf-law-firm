            ...
            <div style={{ marginTop: 28 }}>
              <FieldWrap label="الصفة في القضية" required>
                <Controller name="caseRole" control={control} render={({ field }) => (
                  <PillRadio options={["مدعي", "مدعى عليه", "مشتكي", "متهم", "طرف آخر"]} value={field.value} onChange={field.onChange} />
                )}/>
              </FieldWrap>
            </div>
          </div>
          ...
          <div>
            ...
            <FieldWrap label="نوع القضية" required error={errors.caseType?.message}>
              <Controller name="caseType" control={control} render={({ field }) => (
                <PillRadio
                  options={["تجاري","عمالي","عقاري","أحوال شخصية","جزائي","إداري","ضريبي / زكوي","تنفيذ","تحكيم","إفلاس وإعسار","مقاولات وإنشاءات","أخرى"]}
