...
import { SA_REGIONS, CONTACT, FAQS } from "./data.js";
import { PageHero, Field, PillRadio, Checkbox, validators, CtaStrip, SectionHead, Reveal } from "./components.jsx";
import { SuccessNote, ErrText } from "./pages-main.jsx";
import { validateFiles, partitionFiles, ACCEPT_ATTR, ERR_TYPE, whatsappHref, WHATSAPP_MESSAGE, WHATSAPP_BTN_TEXT } from "@/lib/attachments";
/* ============================================
...
function NewCasePage({ setRoute }) {
  ...
  const onSubmit = async (e) => {
    ...
    setServerError("");
    try {
      // Re-check attachments before building the multipart body.
      const check = validateFiles(data.files, 0);
      if (!check.ok) {
      ...
      // every non-file field travels as a string part
      ...
      const res = await fetch("/api/new-case", {
        method: "POST",
        body: fd, // browser sets multipart/form-data boundary automatically
      });
      const json = await res.json();
      if (!res.ok || !json.ok) throw new Error(json.error || "تعذر إرسال الطلب");
      setSubmitted(true);
      window.scrollTo({ top: 0, behavior: "smooth" });
    } catch (ex) {
      setServerError(ex.message || "تعذر إرسال الطلب");
    } finally {
      setSending(false);
    }
  };
  ...
  const onFiles = (e) => {
    ...
    const existingBytes = data.files.reduce((sum, f) => sum + (f.size || 0), 0);
    const { accepted, typeError, oversize } = partitionFiles(list, existingBytes);
    if (accepted.length) set("files", [...data.files, ...accepted]);
    setFileError(typeError ? ERR_TYPE : "");
    setOversized(oversize); // oversized files were skipped; show WhatsApp fallback
    e.target.value = "";
  };
  const removeFile = (i) => {
        ...
        <div className="container" style={{ maxWidth: 980 }}>
          ...
          <>
              ...
              marginTop: 48
                ...
                {step === 3 && <Step3Files data={data} onFiles={onFiles} removeFile={removeFile} fileError={fileError} oversized={oversized} />}
                {step === 4 && <Step4Declarations data={data} set={set} errors={errors} />}

                <div className="hairline" style={{ margin: "40px 0 28px" }}></div>

                {serverError &&
                <div style={{ marginBottom: 20, padding: "12px 18px", background: "rgba(185,74,72,0.06)", border: "1px solid rgba(185,74,72,0.3)", color: "#b94a48", fontSize: 14 }}>
                  {serverError}
                </div>
                }
                ...
                <div style={{ display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
                  ...
                  <button type="button" className="btn btn-gold" onClick={next}>
                    ...
                    {step === 4 &&
                  <button type="button" className="btn btn-gold" onClick={onSubmit} disabled={sending}>
                        <span>{sending ? "...جارٍ الإرسال" : "إرسال القضية للمراجعة"}</span><span className="arrow"></span>
                      </button>
                  }
                  </div>
                </div>
    ...
    <div className="step-indicator" style={{
      ...
      <style>{`
        @media (max-width: 640px) {
          .step-indicator { grid-template-columns: 1fr 1fr !important; }
          .step-indicator > div { padding: 16px 14px !important; }
        }
      `}</style>
      {steps.map((s, i) => {
...
function Step2Case({ data, set, upd, errors }) {
  ...
  "تنفيذ", "إعسار وإفلاس", "إنشاءات ومقاولات", "أخرى"];
...
/* ---------- Step 3: Files ---------- */
function Step3Files({ data, onFiles, removeFile, fileError, oversized }) {
  const slots = [
  ...
  return (
    <div>
      <StepHeader num="03" title="المرفقات" en="Attachments" lead="حمّل المستندات المتعلقة بالقضية. تُقبل ملفات PDF و Word و Excel و PowerPoint والصور و TXT و ZIP و RAR." />
      ...
      <div role="alert" style={{
        ...
        color: "var(--navy-800)", fontSize: 14, lineHeight: 1.7
      }}>
          {fileError}
        </div>
      }
      ...
      <div role="alert" style={{
        ...
        background: "#fff", border: "1px solid var(--gold-500)",
        borderInlineStart: "3px solid var(--gold-600)",
        color: "var(--navy-800)", fontSize: 14, lineHeight: 1.9
      }}>
          <div style={{ marginBottom: 14 }}>{WHATSAPP_MESSAGE}</div>
          <a
          href={whatsappHref()}
          target="_blank"
          ...
          style={{ display: "inline-flex" }}>
            <span>{WHATSAPP_BTN_TEXT}</span><span className="arrow"></span>
          </a>
        </div>
      }
      ...
      <p style={{ marginTop: 24, fontSize: 13, color: "var(--ink-500)", lineHeight: 1.7 }}>
        الحد الأقصى لحجم الملف الواحد ٤ ميجابايت، وإجمالي المرفقات ١٠ ميجابايت. تُحفظ الملفات بسرّية تامة ولا تُشارك مع أي طرف ثالث.
      </p>
...
function SummaryRow({ k, v, dir, last }) {
  return (
    <div className="summary-row" style={{
      ...
      <div className="eyebrow" style={{ color: "var(--gold-400)", fontSize: 10, minWidth: 0 }}>{k}</div>
      <div style={{ fontSize: 14, color: "var(--ivory-100)", minWidth: 0, overflowWrap: "anywhere", wordBreak: "break-word" }} dir={dir}>{v}</div>
      <style>{`
        @media (max-width: 640px) {
          .summary-row { grid-template-columns: 92px 1fr !important; gap: 14px !important; }
        }
      `}</style>
    </div>);
          ...
          <form onSubmit={submit} style={{
            ...
            padding: "48px clamp(24px, 4vw, 56px)"
              ...
              <button type="submit" className="btn btn-gold" disabled={sending}>
                <span>{sending ? "...جارٍ الإرسال" : "إرسال طلب الاستشارة"}</span><span className="arrow"></span>
              </button>
